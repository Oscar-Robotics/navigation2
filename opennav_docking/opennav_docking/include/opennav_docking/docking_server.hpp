...
class DockingServer : public nav2_util::LifecycleNode
{
  public:
     ...
     */
    bool getCommandToPose(geometry_msgs::msg::Twist &cmd, const geometry_msgs::msg::PoseStamped &pose,
  ...
  protected:
    ...
    // Distance threshold in to detect that the robot went past the target
    double overshoot_distance_threshold_;
    // Maximum number of times the robot will return to staging pose and retry docking
    int max_retries_, num_retries_;
    // Frame used to detect contact with target and to control the robot
    std::string base_frame_, control_frame_, control_sideways_frame_;
